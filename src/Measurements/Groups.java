...
public class Groups {
	...
	// If [MarkovLag] is TRUE, the order of the time points is inverse (from last to first)
	public String[][] getData(Instances sheet, boolean MarkovLag){
        ...
        if(MarkovLag){
			...
			for (int i = 0; i < nrRows-1; i++) {
				// write the class information (last column)
				if (i == 0){
					data[i][nrColumns-1] = sheet.attribute(nrColumns-1).name();
					data[i+1][nrColumns-1] = sheet.instance(i).toString(nrColumns-1);
				} else {
					data[i+1][nrColumns-1] = sheet.instance(i).toString(nrColumns-1);
				}
				// counter set to the first position of the last time point feature group 
				cntColumn = nrColumns-1-nrDims;
				// j --> iterating over each column
				for (int j = 0; j < nrColumns-1; j = j+nrDims) {
					if (i == 0){
						for(int idxDim = 0; idxDim < nrDims; idxDim++){
							data[i][cntColumn+idxDim] = sheet.attribute(j+idxDim).name();
							data[i+1][cntColumn+idxDim] = sheet.instance(i).toString(j+idxDim);
						}
					} else {
						for(int idxDim = 0; idxDim < nrDims; idxDim++){
							data[i+1][cntColumn+idxDim] = sheet.instance(i).toString(j+idxDim);
