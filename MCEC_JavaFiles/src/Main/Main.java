	...
	public static void main(String[] args) throws Exception {
		...
		if(args.length < 3){
			...
			System.out.println("Invalid arguments to run MCEC algorithm. \n " +
					"Format: java -jar <<YOUR-JAR-NAME>>.jar dataset-filename N [0 - without classification; or 1 - with classification");
			System.exit(1);
		}
		...
		int classVerification = Integer.parseInt(args[2]);
		// =======================================================
		...
		boolean classCheck;
		if (classVerification == 1) classCheck = true;
		else classCheck = false;
		// =======================================================
		...
		String[][] dataArray = database.getData(data);
		...
		if (classCheck){		
			...
			// for each time, the last time point is removed until the first is the only one left
